...
impl EventVariant {
    ...
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Self::Unspecified => "EVENT_VARIANT_UNSPECIFIED",
            Self::Created => "EVENT_VARIANT_CREATED",
            Self::Available => "EVENT_VARIANT_AVAILABLE",
            Self::Updated => "EVENT_VARIANT_UPDATED",
            Self::Deleted => "EVENT_VARIANT_DELETED",
            Self::Snapshotted => "EVENT_VARIANT_SNAPSHOTTED",
        }
    }
...
pub mod event_notification_service_server {
    ...
    #[async_trait]
    pub trait EventNotificationService: std::marker::Send + std::marker::Sync + 'static {
        /// CreateStreamConsumer
        ///
        /// Status: BETA
        ///
    ...
    /// A service to receive events for CRUD operations on resources (users, resources etc.)
    #[derive(Debug)]
    pub struct EventNotificationServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> EventNotificationServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for EventNotificationServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "aruna.api.notification.services.v2.EventNotificationService";
    impl<T> tonic::server::NamedService for EventNotificationServiceServer<T> {
