...
pub mod hooks_service_server {
    ...
    #[async_trait]
    pub trait HooksService: std::marker::Send + std::marker::Sync + 'static {
        /// CreateHook
        ///
        /// Status: ALPHA
        ///
    ...
    /// A service that enables automatic Hook scheduling
    #[derive(Debug)]
    pub struct HooksServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> Clone for HooksServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "aruna.api.hooks.services.v2.HooksService";
    impl<T> tonic::server::NamedService for HooksServiceServer<T> {
